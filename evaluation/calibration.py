"""
置信度标定模块：分析 Router 分诊置信度与最终诊断正确率的校准关系。

注意：当前 confidence 来源于 Router Agent（分诊阶段），
Specialist/Summary 不更新此值，因此校准的是「分诊置信度 vs 诊断正确率」，
而非「最终回复置信度 vs 正确率」。若需校准最终回复，需在 Summary 阶段
额外输出置信度字段。
